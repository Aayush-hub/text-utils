...
export default function Textforms() {
  ...
  return (
    <div className="container" style={letstyle}>
      ...
      <h7>
        Total no. of characters: {text.replace(/\s/g, "").length} , Total no. of
        words: {text.split(" ").length}, Reading Minutes:{" "}
        {0.005 * text.split(" ").length}
      </h7>
      <br />
      <button className="btn btn-primary my-3 " onClick={convertUp}>
        Convert to UpperCase
      </button>
      <button className="btn btn-secondary my-3 ms-3" onClick={convertDown}>
        Convert to LowerCase
      </button>
      <button className="btn btn-dark my-3 ms-3" onClick={darkMode}>
        {btnDark}
      </button>
    </div>
  );
